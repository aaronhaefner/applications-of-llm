...
def load_tokenizer_model(model_name: str, device: torch.device) -> tuple:
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained(model_name, token=HFTOKEN).to(
        device
    )
...
def load_and_split_dataset(
    ...
    """Load a local or HF dataset and split into train and test sets.
    ...
    if source == "huggingface":
        dataset = load_dataset(
            dataset_identifier, split="train"
        ).train_test_split(seed=seed)
...
def process_tokenizer(
    tokenizer, train_dataset: dict, test_dataset: dict
) -> tuple:
